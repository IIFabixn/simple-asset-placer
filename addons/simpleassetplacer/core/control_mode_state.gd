...
var _axis_constraints: Dictionary = {"X": false, "Y": false, "Z": false}
...
func process_axis_key_press(
	axis: String, current_time: float, current_position: Vector3 = Vector3.ZERO
) -> void:
	...
	PluginLogger.debug(
		"ControlModeState",
		"Axis %s constraint %s (current: %s)" % [axis, state, get_axis_constraint_string()]
	)
...
func reset() -> void:
	...
	_axis_constraints = {"X": false, "Y": false, "Z": false}
