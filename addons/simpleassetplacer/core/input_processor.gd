...
func handle_navigation(
	nav_input, ui_locked: bool, focus_owner: Control, state: TransformState
) -> void:
...
func _handle_placement_mouse_motion(
	camera: Camera3D, mouse_position: Vector2, state: TransformState
) -> void:
	...
	_services.position_manager.update_position_from_mouse(
		state, camera, mouse_position, 1, false, exclude_nodes
	)


func _handle_transform_mouse_motion(
	camera: Camera3D, mouse_position: Vector2, state: TransformState
) -> void:
	...
	_services.position_manager.update_position_from_mouse(
		state, camera, mouse_position, 1, false, target_nodes
	)
...
func _apply_scale_adjustment(wheel_input: Dictionary) -> void:
	...
	var settings = _get_current_settings()
	var step = _calculate_step(
		settings, "scale_increment", "fine_scale_increment", "large_scale_increment", 0.1, modifiers
	)
...
func _cycle_placement_strategy(state: TransformState) -> void:
	...
	# Update settings
	var settings = (
		state.settings if not state.settings.is_empty() else _get_current_settings().duplicate(true)
	)
...
func _handle_tab_activation(state: TransformState, ignore_focus_lock: bool) -> void:
	...
	if (
		current_scene
		and (
			first_node.is_ancestor_of(current_scene)
			or current_scene == first_node
			or first_node.is_inside_tree()
		)
	):
		_services.transformation_coordinator.start_transform_mode(
			target_nodes, state.dock_reference
		)

		if target_nodes.size() == 1:
			_services.overlay_manager.show_status_message(
				"Transform mode: " + first_node.name, Color.GREEN, 2.0
			)
		else:
			_services.overlay_manager.show_status_message(
				"Transform mode: %d nodes" % target_nodes.size(), Color.GREEN, 2.0
			)
...
func _apply_transform_mode_grid_snap(state: TransformState, pos: Vector3) -> Vector3:
	...
	if _services.position_manager:
		...
		if state.snap.snap_y_enabled:
			...
			state.values.target_position = _services.position_manager._apply_grid_snap(
				state, state.values.target_position
			)
		else:
			# Apply XZ-only grid snap (preserve Y from collision detection)
			state.values.target_position = _services.position_manager._apply_grid_snap_xz_only(
				state, state.values.target_position
			)
...
func _update_position_after_asset_cycle(state: TransformState) -> void:
	...
	_services.position_manager.update_position_from_mouse(
		state, camera, mouse_pos, 1, false, exclude_nodes
	)
