...
func _process_rotation(
	command: TransformCommand, rotation_input: RotationInputState, context: Dictionary
) -> void:
...
func _process_scale(
	command: TransformCommand, scale_input: ScaleInputState, context: Dictionary
) -> void:
	...
	if scale_delta != Vector3.ZERO:
		...
		command.merge_metadata(
			{"scale_multiplier": new_scale, "keyboard_scale": true, "input_mode": "keyboard"}
		)
