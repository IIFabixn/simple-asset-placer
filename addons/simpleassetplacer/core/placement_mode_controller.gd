...
const TransformInputController = preload(
	"res://addons/simpleassetplacer/core/transform_input_controller.gd"
)
...
func confirm_placement(state: TransformState) -> void:
	...
	if continuous_enabled:
		...
		PluginLogger.info(
			PluginConstants.COMPONENT_TRANSFORM, "Asset placed, continuing placement mode"
		)
		_reset_for_next_placement(state)
	else:
		# Exit placement mode
		PluginLogger.info(
			PluginConstants.COMPONENT_TRANSFORM, "Asset placed, exiting placement mode"
		)
...
func process_input(
	camera: Camera3D, state: TransformState, settings: Dictionary, delta: float
) -> void:
...
func update_preview_transform(state: TransformState) -> void:
	...
	var final_rotation = (
		state.values.surface_alignment_rotation + state.values.manual_rotation_offset
	)
	...
	if _services.smooth_transform_manager:
		_services.smooth_transform_manager.set_target_transform(
			preview_mesh, final_position, final_rotation, final_scale
		)
	else:
...
func _setup_preview(
	mesh: Mesh, meshlib, item_id: int, asset_path: String, settings: Dictionary
) -> void:
...
func _place_asset(state: TransformState) -> void:
	...
	if placed_node:
		PluginLogger.info(
			PluginConstants.COMPONENT_TRANSFORM, "Asset placed successfully: " + placed_node.name
		)
		...
		if _services.undo_redo and _services.undo_redo_helper:
			...
			var success = _services.undo_redo_helper.create_placement_undo(
				_services.undo_redo, placed_node, action_name
			)
			if success:
				PluginLogger.info(
					PluginConstants.COMPONENT_TRANSFORM,
					"Undo/redo registered for: " + placed_node.name
				)
			else:
				PluginLogger.error(
					PluginConstants.COMPONENT_TRANSFORM,
					"Failed to register undo/redo for: " + placed_node.name
				)
		else:
			PluginLogger.warning(
				PluginConstants.COMPONENT_TRANSFORM,
				"Undo/redo services not available, node may not persist: " + placed_node.name
			)
	else:
	...
	match parent_mode:
		...
		PluginConstants.PARENT_MODE_SELECTED:
			...
			if selection:
				...
				if selected_nodes.size() > 0:
					...
					PluginLogger.info(
						PluginConstants.COMPONENT_TRANSFORM,
						"Parent mode: selected node '" + selected_node.name + "'"
					)
			...
			if first_node3d:
				PluginLogger.warning(
					PluginConstants.COMPONENT_TRANSFORM,
					"No node selected, using first Node3D: " + first_node3d.name
				)
				return first_node3d
			...
			PluginLogger.warning(
				PluginConstants.COMPONENT_TRANSFORM,
				"No node selected and no Node3D found, falling back to scene root"
			)
			return scene_root
		...
		PluginConstants.PARENT_MODE_CUSTOM:
			...
			if custom_path.is_empty():
				PluginLogger.warning(
					PluginConstants.COMPONENT_TRANSFORM,
					"Custom parent path is empty, using scene root"
				)
				return scene_root
			...
			if target_node:
				PluginLogger.info(
					PluginConstants.COMPONENT_TRANSFORM,
					"Parent mode: custom path '" + target_node.name + "'"
				)
				return target_node
			else:
				PluginLogger.warning(
					PluginConstants.COMPONENT_TRANSFORM,
					"Custom parent path '%s' not found, falling back to scene root" % custom_path
				)
				return scene_root

		PluginConstants.PARENT_MODE_AUTO:
			# Auto-create or reuse container node
			var container_name = settings.get(
				"auto_parent_name", PluginConstants.DEFAULT_AUTO_PARENT_NAME
			)
			...
			if scene_root.has_node(NodePath(container_name)):
				...
				PluginLogger.info(
					PluginConstants.COMPONENT_TRANSFORM,
					"Parent mode: using existing container '" + container_name + "'"
				)
				return container
			else:
				...
				PluginLogger.info(
					PluginConstants.COMPONENT_TRANSFORM,
					"Parent mode: created new container '" + container_name + "'"
				)
				return container
		...
		_:
			...
			PluginLogger.warning(
				PluginConstants.COMPONENT_TRANSFORM,
				"Unknown parent placement mode '%s', using scene root" % parent_mode
			)
			return scene_root
