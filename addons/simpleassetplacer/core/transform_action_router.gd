...
const KeyboardInputProcessor = preload(
	"res://addons/simpleassetplacer/core/keyboard_input_processor.gd"
)
...
func process(params: Dictionary) -> Dictionary:
	...
	# Add base metadata
	var base_metadata := {"timestamp_ms": Time.get_ticks_msec()}
