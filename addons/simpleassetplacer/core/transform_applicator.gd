...
static func apply_transform_state(
	node: Node3D,
	state: TransformState,
	original_transform: Transform3D = Transform3D(),
	skip_position_snap: bool = true
):
	...
	var original_scale = (
		original_transform.basis.get_scale() if original_transform != Transform3D() else Vector3.ONE
	)
...
static func apply_rotation_only(
	node: Node3D, state: TransformState, original_rotation: Vector3 = Vector3.ZERO
):
...
static func apply_scale_only(
	node: Node3D, state: TransformState, original_scale: Vector3 = Vector3.ONE
):
...
static func apply_grid_snap(
	position: Vector3, state: TransformState, use_half_step: bool = false
) -> Vector3:
...
static func force_apply_immediate(
	node: Node3D, state: TransformState, original_transform: Transform3D = Transform3D()
):
...
static func apply_position_to_multiple_nodes(
	nodes: Array, state: TransformState, node_offsets: Dictionary, skip_center_snap: bool = false
):
