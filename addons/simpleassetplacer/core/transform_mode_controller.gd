...
func start(target_nodes: Variant, dock_instance, state: TransformState) -> void:
	...
	var nodes_array = (
		target_nodes if target_nodes is Array else [target_nodes] if target_nodes is Node3D else []
	)
	...
	if valid_nodes.is_empty():
		PluginLogger.warning(
			PluginConstants.COMPONENT_TRANSFORM, "No valid Node3D objects to transform"
		)
		return
	...
	PluginLogger.info(
		...
		"Started transform mode for %d node(s)" % valid_nodes.size()
	)
		...
		if _services.smooth_transform_manager:
			...
			if pos_changed or rot_changed or scale_changed:
				_services.smooth_transform_manager.set_target_transform(
					node, new_position, new_rotation, new_scale
				)
		else:
...
func _transforms_different(a: Transform3D, b: Transform3D) -> bool:
	...
	return not (
		a.origin.distance_to(b.origin) < EPSILON
