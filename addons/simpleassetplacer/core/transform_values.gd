...
func get_final_rotation_degrees() -> Vector3:
	...
	return Vector3(rad_to_deg(rot.x), rad_to_deg(rot.y), rad_to_deg(rot.z))
...
func set_non_uniform_scale(scale_vec: Vector3) -> void:
	...
	non_uniform_multiplier = Vector3(
		max(0.01, scale_vec.x), max(0.01, scale_vec.y), max(0.01, scale_vec.z)
	)
	# Update uniform multiplier to average
	scale_multiplier = (
		(non_uniform_multiplier.x + non_uniform_multiplier.y + non_uniform_multiplier.z) / 3.0
	)
