...
const PlacementModeController = preload(
	"res://addons/simpleassetplacer/core/placement_mode_controller.gd"
)
const TransformModeController = preload(
	"res://addons/simpleassetplacer/core/transform_mode_controller.gd"
)
...
func start_placement_mode(
	...
	exit_any_mode()
	_ensure_undo_redo()
	_placement_controller.start(
		mesh, meshlib, item_id, asset_path, placement_settings, dock_instance, _state()
	)
...
func process_frame_input(
	camera: Camera3D, input_settings: Dictionary = {}, delta: float = 1.0 / 60.0
) -> void:
...
func refresh_settings_from_current() -> void:
	...
	PluginLogger.debug(
		PluginConstants.COMPONENT_TRANSFORM, "Refreshed all settings during active mode"
	)
		...
		for node in target_nodes:
			if node and node.is_inside_tree():
				...
				_services.smooth_transform_manager.apply_transform_immediately(
					node, node.global_position, node.rotation, node.scale
				)
...
func start_placement_from_node3d(node: Node3D, dock_instance = null) -> void:
	...
	if extracted_mesh:
		...
		_services.overlay_manager.show_status_message(
			"Placement mode: " + node.name, Color.GREEN, 2.0
		)
	else:
		_services.overlay_manager.show_status_message(
			"Could not extract mesh from: " + node.name, Color.RED, 3.0
		)
...
func _update_overlay_display(mode: int, state: TransformState) -> void:
	...
	_services.overlay_manager.show_transform_overlay(
		mode, node_name, position, rotation, scale_value, normal_offset, state
	)
