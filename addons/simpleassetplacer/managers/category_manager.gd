...
# Category types
enum CategoryType { FOLDER, CUSTOM_TAG, SPECIAL }  # Auto-detected from folder structure  # User-defined tags  # Favorites, Recent, etc.
...
func load_editor_settings():
	if Engine.is_editor_hint():
		var editor_settings = (
			_services.editor_interface.get_editor_settings() if _services.editor_interface else null
		)
		if editor_settings:
			# Load favorites
...
func save_config_file() -> bool:
	...
	var data = {"tags": custom_tags, "tag_usage": tag_usage, "recently_used": recently_used_tags}
...
func cleanup_orphaned_tags(valid_asset_paths: Array) -> Dictionary:
	...
	if removed_count > 0:
		...
		if removed_tag_names.size() > 0:
			push_warning(
				(
					"CategoryManager: Removed %d unused tags: %s"
					% [removed_tag_names.size(), ", ".join(removed_tag_names)]
				)
			)
...
func cleanup_favorites(valid_asset_paths: Array) -> int:
	...
	if favorites_to_remove.size() > 0:
		...
		categories_updated.emit()
		push_warning(
			"CategoryManager: Cleaned up %d orphaned favorites" % favorites_to_remove.size()
		)
...
func cleanup_recent_assets(valid_asset_paths: Array) -> int:
	...
	if recent_to_remove.size() > 0:
		...
		categories_updated.emit()
		push_warning(
			"CategoryManager: Cleaned up %d orphaned recent assets" % recent_to_remove.size()
		)
