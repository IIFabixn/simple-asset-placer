...
func update_grid_overlay(
	...
	if (
		show_grid
		and (
			current_mode == ModeStateMachine.Mode.PLACEMENT
			or current_mode == ModeStateMachine.Mode.TRANSFORM
		)
	):
		...
		var center = _calculate_grid_center(
			current_mode, transform_state, placement_center, target_nodes
		)
		...
		if should_update_grid(center, center.y, half_step_active):
			...
			_services.overlay_manager.create_grid_overlay(
				center, grid_size, grid_extent, offset, half_step_active, plane_type
			)
...
func should_update_grid(center: Vector3, height: float, half_step_active: bool) -> bool:
	...
	if (
		not _services.overlay_manager.get_grid_overlay()
		or not is_instance_valid(_services.overlay_manager.get_grid_overlay())
	):
		needs_update = true

	# Check if half-step grid state changed
	var half_step_exists = (
		_services.overlay_manager.get_half_step_grid_overlay()
		and is_instance_valid(_services.overlay_manager.get_half_step_grid_overlay())
	)
...
func is_grid_visible() -> bool:
	...
	"""
	return (
		_services.overlay_manager.grid_overlay != null
		and is_instance_valid(_services.overlay_manager.grid_overlay)
	)
