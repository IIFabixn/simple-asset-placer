...
func _init(snapshot: RawInputSnapshot) -> void:
	...
	cycle_next_asset = (
		snapshot.is_key_just_pressed("cycle_next_asset")
		or snapshot.is_key_held_with_repeat("cycle_next_asset")
	)
	cycle_previous_asset = (
		snapshot.is_key_just_pressed("cycle_previous_asset")
		or snapshot.is_key_held_with_repeat("cycle_previous_asset")
	)
