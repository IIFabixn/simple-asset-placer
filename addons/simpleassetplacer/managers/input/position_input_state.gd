...
func _init(snapshot: RawInputSnapshot) -> void:
	...
	height_up_pressed = (
		(height_up_just_pressed or snapshot.is_action_key_held_with_repeat("height_up", "height"))
		and not right_mouse_held
	)
	height_down_pressed = (
		(
			height_down_just_pressed
			or snapshot.is_action_key_held_with_repeat("height_down", "height")
		)
		and not right_mouse_held
	)
	reset_height_pressed = snapshot.is_key_just_pressed("reset_height") and not right_mouse_held
	position_left_pressed = (
		(
			pos_left_just_pressed
			or snapshot.is_action_key_held_with_repeat("position_left", "position")
		)
		and not right_mouse_held
	)
	position_right_pressed = (
		(
			pos_right_just_pressed
			or snapshot.is_action_key_held_with_repeat("position_right", "position")
		)
		and not right_mouse_held
	)
	position_forward_pressed = (
		(
			pos_forward_just_pressed
			or snapshot.is_action_key_held_with_repeat("position_forward", "position")
		)
		and not right_mouse_held
	)
	position_backward_pressed = (
		(
			pos_backward_just_pressed
			or snapshot.is_action_key_held_with_repeat("position_backward", "position")
		)
		and not right_mouse_held
	)
	...
	confirm_action = (
		(snapshot.is_mouse_button_just_pressed("left") and snapshot.is_mouse_in_viewport())
		or snapshot.is_key_just_pressed("confirm")
	)
