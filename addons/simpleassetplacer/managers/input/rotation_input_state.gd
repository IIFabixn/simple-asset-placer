...
func _init(snapshot: RawInputSnapshot) -> void:
	...
	x_pressed = (
		(
			snapshot.is_key_just_pressed("rotate_x")
			or snapshot.is_action_key_held_with_repeat("rotate_x", "rotation")
		)
		and not right_mouse_held
	)
	y_pressed = (
		(
			snapshot.is_key_just_pressed("rotate_y")
			or snapshot.is_action_key_held_with_repeat("rotate_y", "rotation")
		)
		and not right_mouse_held
	)
	z_pressed = (
		(
			snapshot.is_key_just_pressed("rotate_z")
			or snapshot.is_action_key_held_with_repeat("rotate_z", "rotation")
		)
		and not right_mouse_held
	)
