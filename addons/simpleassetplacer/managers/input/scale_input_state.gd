...
func _init(snapshot: RawInputSnapshot) -> void:
	...
	up_pressed = (
		(
			snapshot.is_key_just_pressed("scale_up")
			or snapshot.is_action_key_held_with_repeat("scale_up", "scale")
		)
		and not right_mouse_held
	)
	down_pressed = (
		(
			snapshot.is_key_just_pressed("scale_down")
			or snapshot.is_action_key_held_with_repeat("scale_down", "scale")
		)
		and not right_mouse_held
	)
