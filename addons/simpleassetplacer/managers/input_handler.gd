...
const RawInputSnapshot = preload(
	"res://addons/simpleassetplacer/managers/input/raw_input_snapshot.gd"
)
const RotationInputState = preload(
	"res://addons/simpleassetplacer/managers/input/rotation_input_state.gd"
)
const ScaleInputState = preload(
	"res://addons/simpleassetplacer/managers/input/scale_input_state.gd"
)
const PositionInputState = preload(
	"res://addons/simpleassetplacer/managers/input/position_input_state.gd"
)
const NavigationInputState = preload(
	"res://addons/simpleassetplacer/managers/input/navigation_input_state.gd"
)
...
func should_cycle_next_asset() -> bool:
	return (
		_snapshot.is_key_just_pressed("cycle_next_asset")
		or _snapshot.is_key_held_with_repeat("cycle_next_asset")
	)


func should_cycle_previous_asset() -> bool:
	return (
		_snapshot.is_key_just_pressed("cycle_previous_asset")
		or _snapshot.is_key_held_with_repeat("cycle_previous_asset")
	)
...
func get_mouse_wheel_input(event: InputEventMouseButton) -> Dictionary:
	...
	if (
		_snapshot.is_key_held_for_wheel("height_up")
		or _snapshot.is_key_held_for_wheel("height_down")
	):
		_snapshot.mark_pending_taps(["height_up", "height_down"])
		return _with_wheel_modifiers({"action": "height", "direction": wheel_direction})
