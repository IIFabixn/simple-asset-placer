...
func _create_main_overlay():
	...
	var editor_viewport = (
		_services.editor_interface.get_editor_main_screen() if _services.editor_interface else null
	)
...
func _load_status_overlay_scene():
	...
	if (
		_services
		and _services.placement_strategy_service
		and _status_overlay.has_method("set_placement_strategy_service")
	):
	...
	var viewport_3d = (
		_services.editor_interface.get_editor_viewport_3d(0) if _services.editor_interface else null
	)
	if viewport_3d:
...
func show_transform_overlay(
	...
	var extra_state := {
		"snap_state": snap_state, "modifier_state": modifier_state, "smooth_enabled": smooth_enabled
	}

	# Use the scene's controller method
	_status_overlay.show_transform_info(
		mode, node_name, position, rotation, scale, offset_y, control_mode_state, extra_state
	)
	_current_mode = mode
...
func refresh_overlay_buttons():
	...
	if (
		NodeUtils.is_valid(_toolbar_buttons)
		and _toolbar_buttons.has_method("refresh_button_states")
	):
...
func create_grid_overlay(
	...
	show_half_step: bool = false,
	plane_type: String = "XZ"
):
	...
	if show_half_step:
		_create_half_step_grid(
			center, grid_size * 0.5, grid_extent * 2, offset, editor_root, plane_type
		)


func _draw_xz_grid(
	mesh: ImmediateMesh, center: Vector3, grid_size: float, grid_extent: int, offset: Vector3
):
...
func _create_half_step_grid(
	center: Vector3,
	half_grid_size: float,
	grid_extent: int,
	offset: Vector3,
	editor_root: Node,
	plane_type: String = "XZ"
...
func debug_print_overlay_state():
	...
	PluginLogger.debug(
		"OverlayManager", "  Main Overlay Valid: " + str(NodeUtils.is_valid(_main_overlay))
	)
	PluginLogger.debug(
		"OverlayManager", "  Status Overlay Valid: " + str(NodeUtils.is_valid(_status_overlay))
	)
	PluginLogger.debug(
		"OverlayManager", "  Grid Overlay Valid: " + str(NodeUtils.is_valid(_grid_overlay))
	)
	PluginLogger.debug(
		"OverlayManager",
		"  Half-Step Grid Overlay Valid: " + str(NodeUtils.is_valid(_half_step_grid_overlay))
	)
