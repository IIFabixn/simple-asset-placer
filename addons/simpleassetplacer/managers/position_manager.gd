...
func update_position_from_mouse(
	...
	var surface_offset_position = _apply_surface_offset(
		state.values.base_position, state.values.surface_normal, target_nodes_for_bounds
	)
	...
	if state.snap.snap_enabled or state.snap.snap_y_enabled:
		if state.is_in_transform_mode():
			...
			if state.snap.snap_y_enabled:
				...
				state.values.target_position = _apply_grid_snap_xz_only(
					state, state.values.target_position
				)
	...
	if old_offset != state.values.manual_position_offset:
		PluginLogger.error(
			"PositionManager",
			(
				"BUG: manual_position_offset was changed! Was %s, now %s"
				% [old_offset, state.values.manual_position_offset]
			)
		)
	...
	if state.values.manual_position_offset.length_squared() > 0.001:
		PluginLogger.debug(
			"PositionManager",
			(
				"Position updated to %s, offset preserved: %s"
...
func _apply_grid_snap(state: TransformState, pos: Vector3) -> Vector3:
	...
	var effective_step_x = (
		state.snap.snap_step if not state.snap.use_half_step else state.snap.snap_step * 0.5
	)
	var effective_step_z = (
		state.snap.snap_step if not state.snap.use_half_step else state.snap.snap_step * 0.5
	)
	var effective_step_y = (
		state.snap.snap_y_step if not state.snap.use_half_step else state.snap.snap_y_step * 0.5
	)
...
func _apply_surface_offset(
	hit_position: Vector3, surface_normal: Vector3, transform_nodes: Array = []
) -> Vector3:
...
func adjust_offset_normal_with_modifiers(
	state: TransformState, base_delta: float, modifiers: Dictionary
) -> void:
...
func increase_offset_normal(state: TransformState) -> void:
	...
	var step = (
		state.snap.snap_y_step
		if state.snap.snap_y_enabled
		else state.placement.height_adjustment_step
	)
	adjust_offset_normal(state, step)
...
func move_direction_with_modifiers(
	state: TransformState,
	direction: String,
	base_delta: float,
		...
		if camera:
			...
			if projected_right.length_squared() > 0.01:
				...
				if abs(normal.z) > 0.9:
					...
					return (
						Vector3(sign(projected_right.x), 0, 0)
						if abs(projected_right.x) > 0.01
						else Vector3.RIGHT
					)
				else:
	...
	match axis.to_upper():
		"X":
			...
			rotated_offset = Vector3(
				...
				(
					state.values.manual_position_offset.y * cos_angle
					- state.values.manual_position_offset.z * sin_angle
				),
				(
					state.values.manual_position_offset.y * sin_angle
					+ state.values.manual_position_offset.z * cos_angle
				)
			)
		"Y":
			...
			rotated_offset = Vector3(
				(
					state.values.manual_position_offset.x * cos_angle
					- state.values.manual_position_offset.z * sin_angle
				),
				state.values.manual_position_offset.y,  # Y doesn't change
				(
					state.values.manual_position_offset.x * sin_angle
					+ state.values.manual_position_offset.z * cos_angle
				)
			)
		"Z":
...
func reset_for_new_placement(
	state: TransformState, reset_height_offset: bool = true, reset_position_offset: bool = true
) -> void:
...
func _calculate_position(
	from: Vector3, to: Vector3, additional_config: Dictionary
) -> PlacementStrategy.PlacementResult:
...
func update_smooth_position(state: TransformState, delta: float) -> void:
	...
	if state.values.position.distance_to(state.values.target_position) > 0.01:
		state.values.position = state.values.position.lerp(
			state.values.target_position, _interpolation_speed * delta
		)
...
func configure(state: TransformState, config: Dictionary) -> void:
	...
	if config.has("height_adjustment_step"):
		state.placement.height_adjustment_step = config.get(
...
func update_transform_node_position(
	state: TransformState, transform_node: Node3D, camera: Camera3D, mouse_pos: Vector2
) -> void:
...
func debug_print_position_state(state: TransformState) -> void:
	...
	PluginLogger.debug(
		"PositionManager",
		(
			"Position: %v, Offset Y: %.2f"
			% [state.values.position, state.values.manual_position_offset.y]
		)
	)
...
func get_position_info(state: TransformState) -> Dictionary:
	...
	return {
		...
		(
			...
			and state.values.position.distance_to(state.values.target_position) > 0.01
		)
	}
