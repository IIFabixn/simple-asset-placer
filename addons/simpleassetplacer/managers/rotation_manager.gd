...
func rotate_axis_with_modifiers(
	state: TransformState, axis: String, base_degrees: float, modifiers: Dictionary
):
...
func lerp_to_rotation(state: TransformState, target_rotation: Vector3, weight: float):
	...
	state.values.manual_rotation_offset = state.values.manual_rotation_offset.lerp(
...
func debug_print_rotation(state: TransformState):
	...
	PluginLogger.debug(
		"RotationManager",
		"  Rotation Offset (degrees): X:%.1f Y:%.1f Z:%.1f" % [rot_deg.x, rot_deg.y, rot_deg.z]
	)
	PluginLogger.debug(
		"RotationManager",
		(
			"  Rotation Offset (radians): X:%.3f Y:%.3f Z:%.3f"
			% [
				state.values.manual_rotation_offset.x,
				state.values.manual_rotation_offset.y,
				state.values.manual_rotation_offset.z
			]
		)
	)
