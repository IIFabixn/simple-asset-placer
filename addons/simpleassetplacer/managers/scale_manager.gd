...
func set_scale_multiplier(state: TransformState, multiplier: float):
	...
	state.values.non_uniform_multiplier = Vector3(
		state.values.scale_multiplier, state.values.scale_multiplier, state.values.scale_multiplier
	)
...
func set_non_uniform_multiplier(state: TransformState, multiplier: Vector3):
	...
	state.values.non_uniform_multiplier = Vector3(
		max(0.01, multiplier.x), max(0.01, multiplier.y), max(0.01, multiplier.z)
	)
	# Update uniform scale to average
	state.values.scale_multiplier = (
		(
...
func increase_scale(state: TransformState, amount: float = 0.1):
	...
	PluginLogger.debug(
		"ScaleManager",
		"Increased scale multiplier by " + str(amount) + " to " + str(state.values.scale_multiplier)
	)
...
func decrease_scale(state: TransformState, amount: float = 0.1):
	...
	PluginLogger.debug(
		"ScaleManager",
		"Decreased scale multiplier by " + str(amount) + " to " + str(state.values.scale_multiplier)
	)
...
func adjust_scale_with_modifiers(state: TransformState, base_amount: float, modifiers: Dictionary):
	...
	PluginLogger.debug(
		"ScaleManager",
		"Adjusted scale by " + str(step) + " to " + str(state.values.scale_multiplier)
	)
...
func multiply_scale(state: TransformState, factor: float):
	...
	PluginLogger.debug(
		"ScaleManager",
		"Multiplied scale by " + str(factor) + " to " + str(state.values.scale_multiplier)
	)
...
func scale_axis(state: TransformState, axis: String, amount: float):
	...
	match axis.to_upper():
		"X":
			state.values.non_uniform_multiplier.x = max(
				0.01, state.values.non_uniform_multiplier.x + amount
			)
		"Y":
			state.values.non_uniform_multiplier.y = max(
				0.01, state.values.non_uniform_multiplier.y + amount
			)
		"Z":
			state.values.non_uniform_multiplier.z = max(
				0.01, state.values.non_uniform_multiplier.z + amount
			)
		_:
...
func scale_axis_with_modifiers(
	state: TransformState, axis: String, base_amount: float, modifiers: Dictionary
):
...
func multiply_axis_scale(state: TransformState, axis: String, factor: float):
	...
	match axis.to_upper():
		"X":
			state.values.non_uniform_multiplier.x = max(
				0.01, state.values.non_uniform_multiplier.x * factor
			)
		"Y":
			state.values.non_uniform_multiplier.y = max(
				0.01, state.values.non_uniform_multiplier.y * factor
			)
		"Z":
			state.values.non_uniform_multiplier.z = max(
				0.01, state.values.non_uniform_multiplier.z * factor
			)
		_:
			PluginLogger.warning("ScaleManager", "Invalid axis: " + axis)
			return
...
func clamp_scale(
	state: TransformState, min_multiplier: float = 0.01, max_multiplier: float = 100.0
):
	"""Clamp scale multiplier within specified bounds"""
	state.values.scale_multiplier = clampf(
		state.values.scale_multiplier, min_multiplier, max_multiplier
	)
	state.values.non_uniform_multiplier.x = clampf(
		state.values.non_uniform_multiplier.x, min_multiplier, max_multiplier
	)
	state.values.non_uniform_multiplier.y = clampf(
		state.values.non_uniform_multiplier.y, min_multiplier, max_multiplier
	)
	state.values.non_uniform_multiplier.z = clampf(
		state.values.non_uniform_multiplier.z, min_multiplier, max_multiplier
	)
...
func is_uniform_scale(state: TransformState) -> bool:
	...
	var epsilon = 0.001
	return (
		abs(state.values.non_uniform_multiplier.x - state.values.non_uniform_multiplier.y) < epsilon
		and (
			abs(state.values.non_uniform_multiplier.y - state.values.non_uniform_multiplier.z)
			< epsilon
		)
	)
...
func lerp_to_scale_vector(state: TransformState, target_multiplier: Vector3, weight: float):
	...
	set_non_uniform_multiplier(
		state, state.values.non_uniform_multiplier.lerp(target_multiplier, weight)
	)
...
func debug_print_scale(state: TransformState):
	...
	PluginLogger.debug(
		"ScaleManager", "  Uniform Scale Multiplier: %.3fx" % state.values.scale_multiplier
	)
	PluginLogger.debug(
		"ScaleManager",
		(
			"  Scale Multiplier Vector: X:%.3f Y:%.3f Z:%.3f"
			% [
				state.values.non_uniform_multiplier.x,
				state.values.non_uniform_multiplier.y,
				state.values.non_uniform_multiplier.z
