		...
		PluginConstants.PARENT_MODE_SELECTED:
			...
			if selection:
				...
				if selected_nodes.size() > 0:
					...
					PluginLogger.info(
						"UtilityManager", "Placing under selected node: " + selected_node.name
					)
		...
		PluginConstants.PARENT_MODE_CUSTOM:
			...
			if custom_path.is_empty():
				PluginLogger.warning(
					"UtilityManager", "Custom parent path is empty, using scene root"
				)
				return scene_root
			...
			if target_node:
				PluginLogger.info(
					"UtilityManager", "Placing under custom parent: " + target_node.name
				)
				return target_node
			else:
				PluginLogger.warning(
...
func add_node_to_scene(node: Node, parent: Node) -> void:
	...
	var selection = (
		_services.editor_interface.get_selection() if _services.editor_interface else null
	)
...
func place_asset_in_scene(
	...
	"""
	PluginLogger.info(
		"UtilityManager", "Placing asset: " + asset_path + " at position: " + str(position)
	)
	...
	if transform_state:
		...
		var surface_transform = Transform3D(
			Basis.from_euler(transform_state.values.surface_alignment_rotation), Vector3.ZERO
		)
		var manual_transform = Transform3D(
			Basis.from_euler(transform_state.values.manual_rotation_offset), Vector3.ZERO
		)
	...
	var scale_multiplier = (
		_services.scale_manager.get_scale(transform_state) if transform_state else 1.0
	)
	PluginLogger.info(
		"UtilityManager",
		(
			"Applying scale multiplier: "
			+ str(scale_multiplier)
			+ " (transform_state: "
			+ ("present" if transform_state else "null")
			+ ")"
		)
	)
	...
	_services.smooth_transform_manager.apply_transform_immediately(
		scene_instance, position, final_rotation, final_scale
	)

	PluginLogger.info(
		"UtilityManager",
		(
			"Successfully placed asset as: "
			+ unique_name
			+ " with final scale: "
			+ str(scene_instance.scale)
		)
	)
...
func place_from_meshlib(
	...
	if settings.get("random_rotation", false):
		...
		PluginLogger.debug(
			"UtilityManager",
			"Applied random Y rotation: " + str(rad_to_deg(random_y_rotation)) + " degrees"
		)
	...
	# Apply all transforms immediately without smooth transitions (newly placed objects should snap to final state)
	_services.smooth_transform_manager.apply_transform_immediately(
		mesh_instance, position, final_rotation, final_scale
	)

	PluginLogger.info(
		"UtilityManager",
		(
			"Successfully placed MeshLibrary item as: "
			+ mesh_instance.name
			+ " with final scale: "
...
func place_direct_mesh(
	...
	PluginLogger.info(
		"UtilityManager",
		(
			"Successfully placed direct mesh as: "
			+ mesh_instance.name
			+ " with final scale: "
			+ str(mesh_instance.scale)
		)
...
func place_meshlib_item_in_scene(
	...
	return place_from_meshlib(
		mesh, meshlib, item_id, position, Vector3.ZERO, transform_state, settings, stored_parent
	)


func place_mesh_in_scene(
	mesh: Mesh, position: Vector3, settings: Dictionary = {}, transform_state: TransformState = null
) -> Node3D:
