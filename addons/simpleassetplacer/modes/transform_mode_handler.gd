...
func _process_mouse_rotation(
	...
	var warp_handled: bool = _maybe_warp_cursor(transform_data, "_prev_mouse_pos", mouse_pos, settings, viewport)
	if not warp_handled:
		transform_data["_prev_mouse_pos"] = mouse_pos
...
func _process_mouse_scale(
	...
	var warp_handled: bool = _maybe_warp_cursor(transform_data, "_prev_mouse_pos_scale", mouse_pos, settings, viewport)
	if not warp_handled:
		transform_data["_prev_mouse_pos_scale"] = mouse_pos
...
func _maybe_warp_cursor(transform_data: Dictionary, storage_key: String, mouse_pos: Vector2, settings: Dictionary, viewport: SubViewport) -> bool:
	if not viewport:
		return false
	...
	transform_data[storage_key] = local_center
	return true
