...
const CollisionPlacementStrategy = preload(
	"res://addons/simpleassetplacer/placement/collision_placement_strategy.gd"
)
const PlanePlacementStrategy = preload(
	"res://addons/simpleassetplacer/placement/plane_placement_strategy.gd"
)
...
func set_strategy(strategy_type: String) -> bool:
	...
	match normalized:
		"collision":
			...
			_active_strategy_type = "collision"
			PluginLogger.info(
				PluginConstants.COMPONENT_POSITION, "Switched to collision placement strategy"
			)
			return true
		"plane":
			...
			PluginLogger.info(
				PluginConstants.COMPONENT_POSITION, "Switched to plane placement strategy"
			)
			return true
		_:
			PluginLogger.warning(
				PluginConstants.COMPONENT_POSITION, "Invalid strategy type: %s" % strategy_type
			)
			return false
...
func calculate_position_with_strategy(
	from: Vector3, to: Vector3, strategy_type: String
) -> PlacementStrategy.PlacementResult:
	_initialize_if_needed()
	match strategy_type.to_lower():
		...
		_:
			PluginLogger.warning(
				...
				"Invalid strategy type for calculation: %s" % strategy_type
			)
...
func get_strategy_info() -> Dictionary:
	...
	return {
		...
		"strategies":
		{
			...
			{
				"name":
				_collision_strategy.get_strategy_name() if _collision_strategy else "Collision",
				"type":
				_collision_strategy.get_strategy_type() if _collision_strategy else "collision"
			},
			"plane":
			{
...
func cycle_plane(
	...
	if _active_strategy_type != "plane":
		PluginLogger.warning(
			PluginConstants.COMPONENT_POSITION, "Cannot cycle plane - plane strategy is not active"
		)
		return ""
...
func get_current_plane_data() -> Dictionary:
	...
	return {
		"normal": Vector3.UP, "axis_u": Vector3.RIGHT, "axis_v": Vector3.FORWARD, "axis_index": 1
	}
