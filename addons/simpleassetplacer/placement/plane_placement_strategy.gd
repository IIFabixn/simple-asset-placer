...
# Plane type enum
enum PlaneType { XZ = 0, XY = 1, YZ = 2 }  # Horizontal plane (default)  # Vertical plane facing Z  # Vertical plane facing X
...
func _project_ray_to_plane(
	from: Vector3, ray_dir: Vector3, fallback: Vector3 = Vector3.ZERO, use_fallback: bool = false
) -> Vector3:
...
func _calculate_screen_delta_movement(
	camera: Camera3D, anchor_position: Vector3, screen_delta: Vector2
) -> Variant:
	...
	var world_delta = (
		plane_right * (screen_delta.x * pixel_scale.x) - plane_up * (screen_delta.y * pixel_scale.y)
	)
	return world_delta
...
func _project_vector_onto_plane(
	vector: Vector3, plane_normal: Vector3, fallback: Vector3
) -> Vector3:
...
func _calculate_pixel_to_world_scale(
	camera: Camera3D, anchor_position: Vector3, viewport_size: Vector2
) -> Variant:
...
func _apply_screen_delta(
	camera: Camera3D, anchor_position: Vector3, previous_mouse: Vector2, current_mouse: Vector2
) -> Variant:
...
func calculate_position(from: Vector3, to: Vector3, config: Dictionary) -> PlacementResult:
	...
	if _freeze_mouse_position.x >= 0:
		...
		if mouse_delta < _freeze_threshold:
			...
			return PlacementResult.new(
				last_position, plane_data.normal, false, from.distance_to(last_position)
			)
		else:
	...
	if position == Vector3.ZERO:
		PluginLogger.warning(
			"PlanePlacementStrategy", "Ray projection completely failed (returned zero)"
		)
		return PlacementResult.new(
			last_position, plane_data.normal, false, from.distance_to(last_position)
		)
...
func _initialize_position(
	...
	return PlacementResult.new(
		initial_position, plane_data.normal, false, from.distance_to(initial_position)
	)
...
func initialize_plane_from_position(position: Vector3) -> void:
	...
	PluginLogger.info(
		...
		(
			"Plane initialized at height: %.2f for plane %s from position %v"
			% [plane_height, get_plane_name(), position]
		)
	)
...
func initialize_plane_for_placement() -> void:
	...
	_clear_cache()
	PluginLogger.info(
		PluginConstants.COMPONENT_POSITION, "Plane initialized for placement mode at height: 0.0"
	)
...
func cycle_plane(
	current_position: Vector3 = Vector3.ZERO, mouse_position: Vector2 = Vector2.ZERO
) -> PlaneType:
	...
	if reference_position != Vector3.ZERO:
		...
		PluginLogger.info(
			...
			(
				...
				% [_get_type_name(old_type), get_plane_name(), plane_height, reference_position]
			)
		)
...
func set_plane_type(type: PlaneType) -> void:
	...
	if has_last_position:
		...
		plane_height_locked = true
	PluginLogger.info(
		PluginConstants.COMPONENT_POSITION, "Plane type set to: %s" % get_plane_name()
	)
