...
static func build_settings_ui(
	container: Control, owner_node: Node, settings_data: Dictionary
) -> Dictionary:
	var ui_controls = {}
	...
	for section_key in section_config.keys():
		...
		section_button.add_theme_color_override(
			"font_pressed_color", Color(0.875, 0.875, 0.875, 1.0)
		)
		...
		if section_key == "basic":
			...
			_build_basic_section(
				section_vbox, sections[section_key], owner_node, settings_data, ui_controls
			)
		elif section_key == "reset_behavior":
			# Reset behavior uses checkboxes
			_build_checkbox_section(
				section_vbox, sections[section_key], owner_node, settings_data, ui_controls
			)
		else:
...
static func _build_basic_section(
	container: Control,
	settings: Array,
	owner_node: Node,
	settings_data: Dictionary,
	ui_controls: Dictionary
):
		...
		if setting.type == SettingsDefinition.SettingType.OPTION and ui_controls.has(setting.id):
			...
			if option_button:
				...
				option_button.item_selected.connect(
					func(index: int):
						var selected_value = (
							setting.options[index] if index < setting.options.size() else ""
						)
						_update_dependent_visibility(container, setting.id, selected_value)
				)


static func _update_dependent_visibility(
	parent_container: Control, parent_id: String, parent_value
) -> void:
...
static func _update_dependent_visibility_recursive(
	node: Node, parent_id: String, parent_value
) -> int:
...
static func _build_checkbox_section(
	container: Control,
	settings: Array,
	owner_node: Node,
	settings_data: Dictionary,
...
static func _build_grid_section(
	container: Control,
	settings: Array,
	owner_node: Node,
	settings_data: Dictionary,
