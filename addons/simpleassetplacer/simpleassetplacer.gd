...
# Import context menu
const SceneTreeContextMenu = preload(
	"res://addons/simpleassetplacer/context_menu/scene_tree_context_menu.gd"
)
...
func _connect_placement_settings_to_overlay():
	...
	if (
		dock
		and dock.has_method("get_placement_settings_instance")
		and service_registry
		and service_registry.overlay_manager
	):
		...
		if placement_settings:
			...
			PluginLogger.info(
				PluginConstants.COMPONENT_DOCK, "PlacementSettings reference connected to overlay"
			)
...
func _maybe_show_about_tab() -> void:
	...
	if not editor_settings:
		return
	if (
		editor_settings.has_setting(ABOUT_TAB_SEEN_KEY)
		and editor_settings.get_setting(ABOUT_TAB_SEEN_KEY)
	):
		return
	dock.call_deferred("show_about_tab")
	if (
		not editor_settings.has_setting(ABOUT_TAB_SEEN_KEY)
		or not editor_settings.get_setting(ABOUT_TAB_SEEN_KEY)
	):
...
func _setup_toolbar():
	...
	if ToolbarButtonsScene:
		...
		if toolbar_buttons.has_method("set_transformation_coordinator"):
			toolbar_buttons.set_transformation_coordinator(
				service_registry.transformation_coordinator
			)
...
func _connect_placement_settings_to_toolbar():
	...
	if (
		...
		if placement_settings and toolbar_buttons.has_method("set_placement_settings"):
			...
			PluginLogger.info(
				PluginConstants.COMPONENT_MAIN, "PlacementSettings reference connected to toolbar"
			)
...
func _process(delta: float) -> void:
	...
	if toolbar_buttons and toolbar_buttons.has_method("set_transform_mode_active"):
		toolbar_buttons.set_transform_mode_active(
...
func _input(event: InputEvent) -> void:
	...
	if (
		not service_registry
		or not service_registry.transformation_coordinator
		or not service_registry.settings_manager
	):
		return
	...
	if (
		...
		and service_registry.transformation_coordinator.is_any_mode_active()
	):
		if (
...
func _shortcut_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed:
		...
		if (
			service_registry
			and service_registry.transformation_coordinator
			and service_registry.transformation_coordinator.is_any_mode_active()
		):
			# Check if this key matches any of our plugin keybindings
			if (
				service_registry.settings_manager.is_plugin_key(full_key_string)
				or service_registry.settings_manager.is_plugin_key(key_string)
			):
...
func _forward_3d_gui_input(viewport_camera: Camera3D, event: InputEvent) -> int:
	...
	if (
		...
		service_registry.transformation_coordinator.handle_mouse_motion(
			viewport_camera, event.position
		)
		return EditorPlugin.AFTER_GUI_INPUT_PASS
	...
	if (
		...
		if event.button_index == MOUSE_BUTTON_MIDDLE and event.pressed:
			...
			if (
				service_registry.placement_strategy_service
				and (
					service_registry.placement_strategy_service.get_active_strategy_type()
					== "plane"
				)
			):
				...
				var plane_name = service_registry.placement_strategy_service.cycle_plane(
					current_pos, event.position
				)
				if plane_name:
					PluginLogger.info(
						PluginConstants.COMPONENT_MAIN, "Plane cycled to: " + plane_name
					)
				# Consume the event
	...
	if event is InputEventKey and event.pressed:
		...
		if (
			not service_registry
			or not service_registry.transformation_coordinator
			or not service_registry.transformation_coordinator.is_any_mode_active()
		):
			return EditorPlugin.AFTER_GUI_INPUT_PASS
...
func _on_asset_selected(asset_path: String, mesh_resource: Resource, settings: Dictionary):
	...
	if mesh_resource and mesh_resource is Mesh:
		service_registry.transformation_coordinator.start_placement_mode(
			mesh_resource, null, -1, "", combined_settings, dock
		)
	else:
		service_registry.transformation_coordinator.start_placement_mode(
			null, null, -1, asset_path, combined_settings, dock
		)
