...
static func initialize():
	...
	if (
		...
		and mesh_instance
		and is_instance_valid(mesh_instance)
	):
	...
	if not hidden_container:
		PluginLogger.error(
			PluginConstants.COMPONENT_THUMBNAIL, "Failed to create hidden container!"
		)
		viewport.queue_free()
		viewport = null
		return
	...
	if not light:
		PluginLogger.error(
			PluginConstants.COMPONENT_THUMBNAIL, "Failed to create DirectionalLight3D!"
		)
		return
	...
	if not (viewport and camera and mesh_instance and light):
		PluginLogger.warning(
			PluginConstants.COMPONENT_THUMBNAIL, "Component initialization failed!"
		)
...
static func _evict_lru_entries() -> void:
	...
	PluginLogger.debug(
		...
		"Evicting " + str(entries_to_remove) + " LRU cache entries"
	)
...
static func generate_mesh_thumbnail(asset_path: String) -> ImageTexture:
	...
	if resource is Mesh:
		...
		if not scene_instance is Node3D:
			...
			if scene_container and is_instance_valid(scene_container):
				...
				PluginLogger.error(
					PluginConstants.COMPONENT_THUMBNAIL, "scene_container is null or invalid!"
				)
		...
		if not scene_container or not is_instance_valid(scene_container):
			PluginLogger.debug(
				PluginConstants.COMPONENT_THUMBNAIL,
				"scene_container became invalid after await (plugin disabled during generation)"
			)
			scene_container = null
			_cleanup_generation()
			return null
		...
		var scene_aabb = (
			VisualInstance3D.new().get_aabb() if scene_container.get_child_count() == 0 else AABB()
		)
	...
	if not use_full_scene:
		if not mesh_instance or not is_instance_valid(mesh_instance):
			PluginLogger.error(
				PluginConstants.COMPONENT_THUMBNAIL, "mesh_instance is null or invalid!"
			)
			_cleanup_generation()
			return null
	...
	if viewport and is_instance_valid(viewport):
		...
		PluginLogger.error(
			PluginConstants.COMPONENT_THUMBNAIL, "viewport is null or invalid before rendering!"
		)
		viewport = null
		_cleanup_generation()
		return null
	...
	if not viewport or not is_instance_valid(viewport):
		...
		(
			...
			. debug(
				PluginConstants.COMPONENT_THUMBNAIL,
				(
					"Viewport became invalid during generation for %s (likely plugin disabled or scene changed - this is safe)"
					% asset_path.get_file()
				)
			)
		)
	...
	if not viewport_texture:
		PluginLogger.warning(
			PluginConstants.COMPONENT_THUMBNAIL,
			"Failed to get viewport texture for " + str(asset_path.get_file())
		)
		return null
	...
	if _nodes_processed > MAX_NODES_PER_AABB:
		...
		if not _max_nodes_warning_logged:
			...
			PluginLogger.debug(
				PluginConstants.COMPONENT_THUMBNAIL,
				(
					"AABB calculation reached max nodes: %d (this is normal for complex scenes)"
					% MAX_NODES_PER_AABB
				)
			)
		return combined_aabb
...
static func find_first_mesh_in_node(node: Node) -> Mesh:
	...
	all_mesh_candidates.sort_custom(
		func(a, b):
			var a_vertices = 0
			var b_vertices = 0
...
static func generate_meshlib_thumbnail(meshlib: MeshLibrary, item_id: int = -1) -> ImageTexture:
	...
	if not viewport or not is_instance_valid(viewport):
		...
		(
			...
			. debug(
				PluginConstants.COMPONENT_THUMBNAIL,
				"Viewport became invalid during meshlib generation (likely plugin disabled or scene changed - this is safe)"
			)
		)
		return null
