...
var supported_extensions = [
	"obj", "fbx", "dae", "gltf", "glb", "blend", "tscn", "scn", "tres", "res", "meshlib"
]
...
func setup_ui():
	...
	if not modellib_browser:
		...
		PluginLogger.debug(
			PluginConstants.COMPONENT_DOCK, "ModelLibraryBrowser created successfully"
		)
	...
	if not meshlib_browser:
		...
		PluginLogger.debug(
			PluginConstants.COMPONENT_DOCK, "MeshLibraryBrowser created successfully"
		)
...
func _on_dock_resized():
	...
	if grid_container:
		...
		grid_container.columns = LayoutCalculator.calculate_grid_columns(
			available_width, thumbnail_size
		)
...
func discover_assets():
	...
	PluginLogger.info(
		...
		"AssetScanner found %d assets after filtering" % all_assets.size()
	)
	...
	if modellib_browser:
		PluginLogger.debug(
			PluginConstants.COMPONENT_DOCK, "Passing discovered assets to ModelLibBrowser..."
		)
		...
		PluginLogger.warning(
			PluginConstants.COMPONENT_DOCK, "ModelLibBrowser not initialized, skipping discovery"
		)
...
func update_meshlib_browser():
	...
	if not meshlib_browser:
		PluginLogger.warning(
			PluginConstants.COMPONENT_DOCK,
			"MeshLibBrowser not initialized yet, skipping meshlib update"
		)
		return
...
func _on_asset_selected(asset_info: Dictionary):
	...
	if resource:
		...
		PluginLogger.error(
			PluginConstants.COMPONENT_DOCK, "Failed to load resource: " + asset_info.path
		)
	...
	if category_manager and discovered_assets.size() > 0:
		...
		if cleanup_results["total_items_cleaned"] > 0:
			PluginLogger.info(
				PluginConstants.COMPONENT_DOCK,
				(
					"Cleanup completed: %d tags, %d favorites, %d recent assets removed"
					% [
...
func update_placement_strategy_ui(strategy: String):
	...
	if not placement_settings:
		PluginLogger.warning(
			PluginConstants.COMPONENT_DOCK,
			"Cannot update placement strategy UI - placement_settings not available"
		)
		return
	...
	if not "ui_controls" in placement_settings:
		PluginLogger.warning(
			PluginConstants.COMPONENT_DOCK,
			"Cannot update placement strategy UI - ui_controls not found"
		)
		return
	...
	if not ui_controls.has("placement_strategy"):
		PluginLogger.warning(
			PluginConstants.COMPONENT_DOCK,
			"Cannot update placement strategy UI - control not found"
		)
		return
	...
	if not option_button is OptionButton:
		PluginLogger.warning(
			PluginConstants.COMPONENT_DOCK,
			"Cannot update placement strategy UI - control is not OptionButton"
		)
		return
	...
	for setting in all_settings:
		if setting.id == "placement_strategy" and setting.options:
			...
			if selected_index >= 0:
				...
				PluginLogger.info(
					PluginConstants.COMPONENT_DOCK, "Updated placement strategy UI to: " + strategy
				)
			else:
				PluginLogger.warning(
					PluginConstants.COMPONENT_DOCK, "Strategy not found in options: " + strategy
				)
			return

	PluginLogger.warning(
		PluginConstants.COMPONENT_DOCK, "Could not find placement_strategy setting definition"
	)
