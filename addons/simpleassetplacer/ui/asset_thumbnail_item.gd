...
func _init(
	p_meshlib: MeshLibrary = null,
	p_item_id: int = -1,
	p_thumbnail_size: int = LayoutCalculator.THUMBNAIL_SIZE_DEFAULT
):
	if p_meshlib != null:
		meshlib = p_meshlib
		item_id = p_item_id
...
static func create_for_asset(
	p_asset_info: Dictionary, p_thumbnail_size: int = LayoutCalculator.THUMBNAIL_SIZE_DEFAULT
) -> AssetThumbnailItem:
...
func _add_category_badges(vbox: VBoxContainer):
	...
	if custom_tags.size() > 0 and tags_to_show.size() < 2:
		tags_to_show.append(
			{"text": custom_tags[0], "color": Color(0.4, 0.8, 0.4), "type": "custom"}
		)
	...
	if tags_to_show.size() < 2:
		if custom_tags.size() > 1:
			tags_to_show.append(
				{"text": custom_tags[1], "color": Color(0.4, 0.8, 0.4), "type": "custom"}
			)
		elif folder_cats.size() > 0:
			tags_to_show.append(
				{"text": folder_cats[0], "color": Color(0.5, 0.7, 1.0), "type": "folder"}
			)
...
func set_selected(selected: bool):
	...
	if is_selected:
		...
		if not selection_border:
			...
			selection_border.texture = EditorInterface.get_editor_theme().get_icon(
				"GuiChecked", "EditorIcons"
			)
