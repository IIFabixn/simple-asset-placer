...
func update_grid_columns(available_width: float):
	if items_grid:
		...
		var columns = LayoutCalculator.calculate_grid_columns(
			available_width - 48, thumbnail_size, 12, 20
		)
...
func set_discovered_assets(assets: Array):
	...
	PluginLogger.info(
		...
		"ModelLibBrowser: Received %d pre-discovered assets" % assets.size()
	)
	...
	for asset in assets:
		...
		if category_manager:
			asset_with_categories["folder_categories"] = category_manager.extract_folder_categories(
				asset.path
			)
	...
	PluginLogger.info(
		PluginConstants.COMPONENT_DOCK,
		"ModelLibBrowser: Processed %d assets with categories" % discovered_assets.size()
	)
...
func _on_category_filter_changed(index: int):
	...
	if _services and _services.settings_manager:
		_services.settings_manager.set_plugin_setting(
			"last_model_category", current_category_filter
		)
...
func _on_context_menu_requested(asset_item: AssetThumbnailItem, position: Vector2):
	...
	if recent_tags.size() > 0:
		...
		for tag in recent_tags:
			...
			menu_actions[menu_id] = {
				"type": "tag", "name": tag, "assigned": is_assigned, "asset_path": asset_path
			}
			menu_id += 1
