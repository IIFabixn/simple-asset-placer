...
@onready
var position_label: Label = $OverlayControl/StatusPanel/ContentVBox/TransformHBox/PositionLabel
@onready
var rotation_label: Label = $OverlayControl/StatusPanel/ContentVBox/TransformHBox/RotationLabel
...
const PlacementStrategyService = preload(
	"res://addons/simpleassetplacer/placement/placement_strategy_service.gd"
)
...
func _assign_optional_nodes() -> void:
	grid_badge_label = (
		get_node_or_null("OverlayControl/StatusPanel/ContentVBox/StateBadgeHBox/GridBadgeLabel")
		as Label
	)
	y_snap_badge_label = (
		get_node_or_null("OverlayControl/StatusPanel/ContentVBox/StateBadgeHBox/YSnapBadgeLabel")
		as Label
	)
	rotation_snap_badge_label = (
		get_node_or_null(
			"OverlayControl/StatusPanel/ContentVBox/StateBadgeHBox/RotationSnapBadgeLabel"
		)
		as Label
	)
	scale_snap_badge_label = (
		get_node_or_null(
			"OverlayControl/StatusPanel/ContentVBox/StateBadgeHBox/ScaleSnapBadgeLabel"
		)
		as Label
	)
	half_step_badge_label = (
		get_node_or_null("OverlayControl/StatusPanel/ContentVBox/StateBadgeHBox/HalfStepBadgeLabel")
		as Label
	)
	surface_align_badge_label = (
		get_node_or_null(
			"OverlayControl/StatusPanel/ContentVBox/StateBadgeHBox/SurfaceAlignBadgeLabel"
		)
		as Label
	)
	smooth_badge_label = (
		get_node_or_null("OverlayControl/StatusPanel/ContentVBox/StateBadgeHBox/SmoothBadgeLabel")
		as Label
	)
	cursor_warp_badge_label = (
		get_node_or_null(
			"OverlayControl/StatusPanel/ContentVBox/StateBadgeHBox/CursorWarpBadgeLabel"
		)
		as Label
	)
	...
	fine_modifier_label = (
		get_node_or_null(
			"OverlayControl/StatusPanel/ContentVBox/ModifierBadgeHBox/FineModifierLabel"
		)
		as Label
	)
	large_modifier_label = (
		get_node_or_null(
			"OverlayControl/StatusPanel/ContentVBox/ModifierBadgeHBox/LargeModifierLabel"
		)
		as Label
	)
	reverse_modifier_label = (
		get_node_or_null(
			"OverlayControl/StatusPanel/ContentVBox/ModifierBadgeHBox/ReverseModifierLabel"
		)
		as Label
	)

	numeric_container = (
		get_node_or_null("OverlayControl/StatusPanel/ContentVBox/NumericHBox") as HBoxContainer
	)
	if numeric_container:
...
func show_transform_info(
	...
	if position_label:
		position_label.text = (
			"Pos: X: %.3f  Y: %.3f  Z: %.3f" % [position.x, position.y, position.z]
		)
	...
	if keybinds_label:
		if offset_y != 0.0:
			...
			var wheel_hint = (
				"Mouse Wheel (Adjust; %s reverse, %s fine, %s large)"
				% [reverse_key, fine_key, large_key]
			)
			...
			if mode == ModeStateMachine.Mode.PLACEMENT:
				keybinds_label.text = (
					"%s/%s (Quick Height)  X/Y/Z (Axis Constraints)  %s%s  ENTER (Place)"
					% [height_up, height_down, wheel_hint, plane_hint]
				)
			else:  # transform mode
				keybinds_label.text = (
					"%s/%s (Quick Height)  X/Y/Z (Axis Constraints)  %s%s  ENTER (Confirm)"
					% [height_up, height_down, wheel_hint, plane_hint]
				)
...
func show_status_message(message: String, color: Color = Color.GREEN) -> void:
	...
	if _services:
		var combined_settings: Dictionary = (
			_services.settings_manager.get_combined_settings() if _services.settings_manager else {}
		)
		snap_state = {
...
func update_numeric_state(action_name: String, input_string: String) -> void:
	if (
		not numeric_container
		or not numeric_action_label
		or not numeric_value_label
		or not numeric_glyph_label
	):
		return
	...
	if has_value:
		...
		numeric_value_label.text = (
			input_string.strip_edges() if input_string.strip_edges() != "" else "_"
		)
...
func _update_numeric_state_from_extra(numeric_state: Dictionary) -> void:
	if numeric_state.is_empty():
		return
	update_numeric_state(
		numeric_state.get("action_name", ""), numeric_state.get("input_string", "")
	)
...
func _set_numeric_highlight(active: bool) -> void:
	...
	numeric_glyph_label.add_theme_color_override(
		"font_color", glyph_color if active else inactive_color
	)
	numeric_action_label.add_theme_color_override(
		"font_color", glyph_color if active else inactive_color
	)
	numeric_value_label.add_theme_color_override(
		"font_color", glyph_color if active else inactive_color
	)
