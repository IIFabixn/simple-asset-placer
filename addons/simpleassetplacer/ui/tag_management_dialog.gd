...
func _update_statistics() -> void:
	...
	var stats_text = (
		"📊 Assets: %d total  •  ✓ %d tagged  •  ○ %d untagged  •  🏷️ %d tags"
...
func _get_asset_identifier(asset_info: Dictionary) -> String:
	...
	if asset_info.has("path"):
		...
		PluginLogger.error(
			"TagManagementDialog", "Asset info missing required keys: " + str(asset_info.keys())
		)
		return ""
...
func _on_bulk_add_tags() -> void:
	...
	PluginLogger.info(
		"TagManagementDialog",
		"Bulk add: %d tags to %d assets" % [selected_tags.size(), selected_asset_paths.size()]
	)
	...
	if changes_made:
		...
		# Show confirmation
		var msg = (
			"Added %d tag(s) to %d asset(s)" % [selected_tags.size(), selected_asset_paths.size()]
		)
...
func _on_bulk_remove_tags() -> void:
	...
	PluginLogger.info(
		"TagManagementDialog",
		"Bulk remove: %d tags from %d assets" % [selected_tags.size(), selected_asset_paths.size()]
	)
	if selected_tags.is_empty() or selected_asset_paths.is_empty():
		return
	...
	if changes_made:
		...
		# Show confirmation
		var msg = (
			"Removed %d tag(s) from %d asset(s)"
			% [selected_tags.size(), selected_asset_paths.size()]
		)
		_show_notification(msg)
...
func _on_rename_tag() -> void:
	...
	var rename_action = func():
		...
		_show_notification(
			"Renamed tag '%s' to '%s' (%d assets)" % [old_tag, new_tag, assets_with_tag.size()]
		)
		dialog.queue_free()
...
func _on_merge_tags() -> void:
	...
	var merge_action = func():
		...
		_show_notification(
			(
				"Merged %d tags into '%s' (%d assets affected)"
				% [tags_to_merge.size(), target_tag, affected_assets]
			)
		)
		dialog.queue_free()
...
func _on_delete_tag() -> void:
	...
	dialog.confirmed.connect(
		func():
			...
			_show_notification(
				"Deleted %d tag(s) from %d asset(s)" % [selected_tags.size(), affected_assets]
			)
			dialog.queue_free()
	)
...
func _on_create_new_tag() -> void:
	...
	PluginLogger.debug(
		"TagManagementDialog",
		"Creating new tag with %d assets selected" % selected_asset_paths.size()
	)
	if selected_asset_paths.size() > 0:
		...
		func():
			...
			if not selected_asset_paths.is_empty():
				PluginLogger.info(
					"TagManagementDialog",
					(
						"Assigning tag '%s' to %d selected assets"
						% [new_tag, selected_asset_paths.size()]
					)
				)
				...
				success_dialog.dialog_text = (
					"✅ Tag '%s' created and added to %d asset(s)!\n\nYou can now select more assets and tags to perform bulk operations."
					% [new_tag, selected_asset_paths.size()]
				)
				...
				PluginLogger.info(
					"TagManagementDialog",
					"No assets selected, initializing tag '%s' in system only" % new_tag
				)
				...
				PluginLogger.info(
					"TagManagementDialog",
					"Tag '%s' now visible in tag list with 0 usage, ready to assign" % new_tag
				)
