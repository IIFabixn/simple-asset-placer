		...
		if node3d_nodes.is_empty():
			...
			if _services and _services.overlay_manager:
				_services.overlay_manager.show_status_message(
					"Select a Node3D to enter Transform Mode", Color.YELLOW, 2.0
				)
			transform_mode_button.set_pressed_no_signal(false)
			return
...
func _on_reset_transforms_pressed() -> void:
	...
	if not _coordinator.is_any_mode_active():
		if _services and _services.overlay_manager:
			_services.overlay_manager.show_status_message(
				"No active mode. Enter placement or transform mode first.", Color.YELLOW, 2.0
			)
		return
...
func _update_smooth_transforms_button() -> void:
	...
	if smooth_transforms_button.button_pressed != enabled:
		var was_connected = smooth_transforms_button.toggled.is_connected(
			_on_smooth_transforms_toggled
		)
		if was_connected:
...
func _update_button_tooltips() -> void:
	...
	if placement_mode_button:
		placement_mode_button.tooltip_text = (
			"Cycle Placement Strategy (%s)\n🎯 Collision-based\n📐 Plane-based" % placement_key
		)

	# Update Transform Mode tooltip with actual keybind
	if transform_mode_button:
		transform_mode_button.tooltip_text = (
			"Transform Mode (%s)\nEdit placed objects\nPosition, Rotation, Scale" % transform_key
		)

	if surface_align_button:
		surface_align_button.tooltip_text = (
			"Toggle Surface Alignment\nAlign placement rotation to surface normals\nCurrent: %s"
			% ("On" if surface_align_button.button_pressed else "Off")
		)

	if smooth_transforms_button:
		smooth_transforms_button.tooltip_text = (
			"Toggle Smooth Transforms\nLerp preview and transform updates\nCurrent: %s"
			% ("On" if smooth_transforms_button.button_pressed else "Off")
		)

	if cursor_warp_button:
		cursor_warp_button.tooltip_text = (
			"Toggle Cursor Warp\nKeep mouse centered during transforms\nCurrent: %s"
			% ("On" if cursor_warp_button.button_pressed else "Off")
		)
...
func set_placement_settings(settings: PlacementSettings) -> void:
	...
	if (
		placement_settings_ref
		and placement_settings_ref.settings_changed.is_connected(_on_settings_changed)
	):
...
func set_transform_mode_active(active: bool) -> void:
	...
	if transform_mode_button:
		...
		if transform_mode_button.button_pressed != active:
			# Disconnect signal, update, then reconnect
			var was_connected = transform_mode_button.toggled.is_connected(
				_on_transform_mode_toggled
			)
			if was_connected:
