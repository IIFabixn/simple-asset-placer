...
func calculate_constrained_position(
	camera: Camera3D, mouse_pos: Vector2, current_pos: Vector3, control_mode: ControlModeState
) -> Vector3:
	"""
	...
	var origin = (
		control_mode.get_constraint_origin()
	...
	var active_count = (
		int(constraints.get("X", false))
		+ int(constraints.get("Y", false))
		+ int(constraints.get("Z", false))
	)
...
func _calculate_line_constraint(
	ray_origin: Vector3, ray_dir: Vector3, origin: Vector3, constraints: Dictionary
) -> Vector3:
...
func _calculate_plane_constraint(
	ray_origin: Vector3, ray_dir: Vector3, origin: Vector3, constraints: Dictionary
) -> Vector3:
...
func project_ray_onto_line(
	ray_origin: Vector3, ray_dir: Vector3, line_origin: Vector3, line_dir: Vector3
) -> float:
	"""
