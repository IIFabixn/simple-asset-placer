...
enum Severity { INFO, WARNING, ERROR, CRITICAL }  # Informational message (green)  # Warning that doesn't stop operation (yellow)  # Error that stops current operation (red)  # Critical error requiring attention (dark red)
...
static func _connect_dialog_cleanup(dialog: Window) -> void:
	...
	var callbacks = {"confirmed": Callable(), "canceled": Callable()}
...
static func _connect_dialog_with_callbacks(
	dialog: Window, on_confirm: Callable, on_cancel: Callable
) -> void:
...
static func show_error(
	component: String, title: String, message: String, details: String = ""
) -> void:
...
static func log_and_notify(
	component: String, message: String, severity: Severity = Severity.INFO
) -> void:
...
static func handle_resource_load_error(
	component: String, path: String, error_code: int = -1
) -> void:
...
static func handle_key_conflict(
	component: String, key: String, action1: String, action2: String
) -> void:
...
static func handle_thumbnail_error(
	component: String, asset_path: String, reason: String = ""
) -> void:
...
static func handle_settings_error(
...
static func handle_invalid_setting_value(
	component: String, setting_name: String, value: String, constraint: String
) -> void:
...
static func show_confirmation(
	title: String, message: String, on_confirm: Callable, on_cancel: Callable = Callable()
) -> void:
...
static func suggest_recovery(
	component: String, problem: String, suggestions: Array[String]
) -> void:
...
static func get_error_handler_status() -> Dictionary:
	...
	return {"initialized": _editor_interface != null, "can_show_dialogs": _editor_interface != null}
