...
static func configure_multipliers(large: float = 5.0, fine: float = 0.1) -> void:
	...
	PluginLogger.debug(
		"IncrementCalculator",
		(
			"Configured multipliers - Large: %.1f, Fine: %.2f"
			% [large_increment_multiplier, fine_increment_multiplier]
		)
	)
...
static func get_configuration() -> Dictionary:
	...
	return {
		"large_multiplier": large_increment_multiplier, "fine_multiplier": fine_increment_multiplier
	}
...
static func calculate_with_preset(
	preset_name: String, base_step: float, modifiers: Dictionary
) -> float:
...
static func calculate_clamped_step(
	base_step: float, modifiers: Dictionary, min_step: float = 0.0, max_step: float = INF
) -> float:
...
static func debug_print_step_calculation(base_step: float, modifiers: Dictionary) -> void:
	...
	PluginLogger.debug(
		"IncrementCalculator",
		"  Large Active: %s (x%.1f)" % [is_large_active(modifiers), large_increment_multiplier]
	)
	PluginLogger.debug(
		"IncrementCalculator",
		"  Fine Active: %s (x%.2f)" % [is_fine_active(modifiers), fine_increment_multiplier]
	)
